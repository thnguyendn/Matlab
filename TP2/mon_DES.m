...
% tao 1 cai cle 64 bits ngau nhien
if strcmp(cle_entre, '"'),
...
% Tao cac bit parité o cac vi tri 8, 16, 24, 32...
if size(K0,2) == 64,
	for i=1:8,
...
% Tao L0 voi cac phan tu tu ma tran K0(1x64). Vi tri cac phan tu nay
% chinh la cac phan tu cua ma tran PC1g(1x28), tham so dau tien chi
% ligne nao ma minh muon lay tu ma tran K0
L0 = K0(1,PC1g);
R0 = K0(1,PC1d);
...
for i=1:16,
	...
	%Permutation circulaire cho L0
	tmp = L0(1:vi);
	L0(1:end-vi) = L0(vi+1:end);
	L0(end-vi+1:end) = tmp;

    %Permutation circulaire cho R0
	tmp = R0(1:vi);
